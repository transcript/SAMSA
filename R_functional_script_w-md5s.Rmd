...
```{r working_directory}
setwd("~/Desktop/Projects/Lab Stuff/location_of_functional_summary_files/")
```
...
```{r file_listing}
control_files <- list.files( pattern = "control_*", full.names = T, recursive = FALSE)
control_names = ""
...
exp_files <- list.files( pattern = "experimental_*", full.names = T, recursive = FALSE)
exp_names = ""
...
all_table[, " "] <- 0
all_table <- merge(all_table, exp_table_filtered, by = c(0,1), all = TRUE)
...
# Although up to this point we have included all of the functions found within the sample, this proves very difficult to display on a graph.  Therefore, this next code chunk filters down to only the top 30 protein functions within each sample, grouping all others under an "Other" catchall category.
all_table["Other",] <- colSums(all_table[30:nrow(all_table),])
...
dds <- DESeq(dds)
res <- results(dds, contrast = c("condition", "control", "experimental"))
temp_func_results <- data.frame(res)
temp_func_results$Function <- complete_table$Simplified_Function
func_results <- temp_func_results[order(temp_func_results$padj),]

res <-res[order(res$padj,decreasing = TRUE),]
plotMA(dds, ylim=c(-5,5), main="DESeq2 comparison of functional annotations")

write.table(func_results, file = "DESeq_functional_output_with_md5s.tab", append = FALSE, quote = FALSE, sep = "\t", row.names = TRUE, col.names = TRUE)
```
