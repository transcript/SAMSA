...
```{r working_directory}
setwd("~/Desktop/Projects/Lab Stuff/location_of_organism_results/")
```
...
for (name in control_files) {
  control_names <- c(control_names, unlist(strsplit(name, split='_', fixed=TRUE))[4])} #change the "4" to a different part of the name if not following standard naming conventions as above
...
for (name in exp_files) {
  exp_names <- c(exp_names, unlist(strsplit(name, split='_', fixed=TRUE))[4])} #again, change 4 if non-standard file names are used
...
org_results <- data.frame(res[order(res$baseMean, decreasing = TRUE),])
write.table(org_results, file = "DESeq_organism_output.tab", append = FALSE, quote = FALSE, sep = "\t", row.names = TRUE, col.names = TRUE)

write.table(complete_table, file = "organism_complete_table.tab", append = FALSE, quote = FALSE, sep = "\t", row.names = TRUE, col.names = TRUE)
```
